...
### Iteration 1:
- Phase 1: service_lifecycle.py, service_status.py route, app.py lifespan integration
- Phase 2: claude_ws_client.py, opencode_client.py, provider_router.py, chat_sessions.py rewrite
- Phase 4: context_assembler.py
- Phase 5: provider_service.py model discovery update
- Phase 6: architecture.html service status indicators + health polling
- Phase 7: Companion verification (no changes needed, bun discovery in lifecycle manager)
- Phase 8: app.py integration, config.py chat settings, chat_session model update

### Iteration 2:
- Phase 3: diagram_watcher.py rewritten from polling to watchdog (26 new tests)
- Phase 8 (completion): mcp_server.py _start_opencode_serve() removed, replaced with lifecycle query
- Legacy removal: keepalive_manager.py deleted, chat.py deleted, chat_sessions.py simplified
- app.py: removed keepalive/chat imports, bumped to v1.4.1

### Tests: 362 unit tests passing (48 new total), 0 failures
...
| Step | Phase | Files | Dependencies | Effort | Status |
|------|-------|-------|-------------|--------|--------|
| 1 | Phase 1 | `service_lifecycle.py` | None | Medium | DONE |
| 2 | Phase 1 | `routes/service_status.py`, `config.py` changes | Step 1 | Small | DONE |
| 3 | Phase 1 | `app.py` lifespan changes | Steps 1-2 | Small | DONE |
| 4 | Phase 3 | `diagram_watcher.py` rewrite | `watchdog` package | Medium | DONE |
| 5 | Phase 4 | `context_assembler.py` | None | Small | DONE |
| 6 | Phase 2 | `claude_ws_client.py` | `websockets`, `httpx`, Step 1 | Large | DONE |
| 7 | Phase 2 | `opencode_client.py` | `httpx`, Step 1 | Medium | DONE |
| 8 | Phase 2 | `provider_router.py` | Steps 6+7 | Medium | DONE |
| 9 | Phase 2 | `chat_service.py` rewrite, `chat_sessions.py` update | Steps 5+8 | Medium | DONE |
| 10 | Phase 5 | `provider_service.py` model discovery | Steps 1+7 | Small | DONE |
| 11 | Phase 6 | `architecture.html` frontend changes | Steps 1+9 | Medium | DONE |
| 12 | Phase 7 | Companion fork verification + Bun check | Step 1 | Small | DONE |
| 13 | Phase 8 | Final integration, `mcp_server.py` cleanup | All above | Medium | DONE |
