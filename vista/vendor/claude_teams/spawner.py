...
def _win_to_wsl_path(win_path: str) -> str:
    """Convert a Windows/MSYS path to a WSL ``/mnt/...`` path.

    Handles both native (``C:\\Users\\...``) and MSYS (``/c/Users/...``) forms.
    """
    p = win_path.replace("\\", "/")
    # Native Windows: C:/Users/... → /mnt/c/Users/...
    if len(p) >= 2 and p[1] == ":":
        return f"/mnt/{p[0].lower()}{p[2:]}"
    # MSYS/Git-Bash: /c/Users/... → /mnt/c/Users/...
    if len(p) >= 3 and p[0] == "/" and p[1].isalpha() and p[2] == "/":
        return f"/mnt/{p[1].lower()}{p[2:]}"
    return p
...
def use_tmux_windows() -> bool:
    ...
    Always True on Windows (WSL tmux has no parent pane to split).
    """
    return _IS_WINDOWS or os.environ.get("USE_TMUX_WINDOWS") is not None


def _tmux_cmd(*args: str) -> list[str]:
    """Return the command prefix for invoking tmux.

    On Windows, tmux runs inside WSL Ubuntu, so we prefix with
    ``wsl -d Ubuntu -- tmux``.  WSL's argument passing strips bare
    ``#{...}`` (interprets ``#`` as bash comment), so we escape
    ``#`` → ``\\#`` in any arg containing tmux format variables.
    On Unix, plain ``tmux`` suffices.
    """
    if _IS_WINDOWS:
        escaped = []
        for a in args:
            if "#{" in a:
                a = a.replace("#", "\\#")
            escaped.append(a)
        return ["wsl", "-d", "Ubuntu", "--", "tmux", *escaped]
    return ["tmux", *args]


def _ensure_tmux_server() -> None:
    """Ensure a tmux server is running; create one if needed."""
    check = subprocess.run(
        _tmux_cmd("has-session"),
        capture_output=True,
        timeout=10,
    )
    if check.returncode != 0:
        subprocess.run(
            _tmux_cmd("new-session", "-d", "-s", "claude-teams"),
            check=True,
            timeout=10,
        )
    # Keep panes alive after process exits so output is preserved
    subprocess.run(
...
def build_tmux_spawn_args(command: str, name: str) -> list[str]:
    ...
    return _tmux_cmd("split-window", "-dP", "-F", "#{pane_id}", command)
...
def skip_permissions() -> bool:
    ...
    # TODO: revert to env-var check once testing is complete
    return True
...
def build_spawn_command(
    ...
    if _IS_WINDOWS:
        # tmux runs inside WSL; translate Windows paths to /mnt/... paths
        # and ensure the binary has .exe extension for WSL interop.
        cwd = _win_to_wsl_path(cwd)
        binary = _win_to_wsl_path(binary)
        if not binary.lower().endswith(".exe"):
            binary += ".exe"

    # Log file for capturing agent output
    log_dir = Path.home() / ".claude" / "teams" / team_name / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = str(log_dir / f"{member.name}.log")
    if _IS_WINDOWS:
        log_file = _win_to_wsl_path(log_file)

    # On Windows, WSL inline env vars (VAR=x cmd) aren't forwarded to .exe
    # binaries via WSL interop. Use export + WSLENV to bridge them.
    if _IS_WINDOWS:
        env_prefix = (
            "export CLAUDECODE=1 CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 "
            "WSLENV=CLAUDECODE:CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS && "
        )
    else:
        env_prefix = "CLAUDECODE=1 CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 "
...
def build_opencode_attach_command(
    ...
    if _IS_WINDOWS:
        ...
        if not binary.lower().endswith(".exe"):
            binary += ".exe"
        resolved_cwd = _win_to_wsl_path(cwd)
    return (
        f"{shlex.quote(binary)} attach "
        f"{shlex.quote(server_url)} "
        f"-s {shlex.quote(session_id)} "
        f"--dir {shlex.quote(resolved_cwd)}"
    )
...
def kill_tmux_pane(pane_id: str) -> None:
    if pane_id.startswith("@"):
        subprocess.run(_tmux_cmd("kill-window", "-t", pane_id), check=False, timeout=10)
        return
    subprocess.run(_tmux_cmd("kill-pane", "-t", pane_id), check=False, timeout=10)
